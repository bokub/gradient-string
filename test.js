...
test('supports aliases', t => {
	...
	// Background color
	const bgOptions = {bg: true};
	t.is(g.cristal('Hello world', bgOptions), g('#bdfff3', '#4ac29a')('Hello world', bgOptions));
	t.is(g.pastel('Hello world', bgOptions), g('#74ebd5', '#74ecd5')('Hello world', {...bgOptions, interpolation: 'hsv', hsvSpin: 'long'}));
});

test('multiline option works fine', t => {
	t.is(g('orange', 'purple')('hello\nworld'), g('orange', 'purple')('hello') + '\n' + g('orange', 'purple')('world'));
	t.is(g.atlas('abc\n\ndef'), g.atlas('abc') + '\n\n' + g.atlas('def'));
	t.not(g.rainbow('hi\nworld'), g.rainbow('hi') + '\n' + g.rainbow('world'));
});
